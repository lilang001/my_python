...
from apps import electricity_meter as electricity
dir_path = os.path.dirname(os.path.realpath(__file__))


class electricity_meter(unittest.TestCase):
    def setUp(self):
    ...
    def test_01(self):
        """推送正式设备数据"""
        data = electricity.JSONV2_Request(self.NdeviceId)
        ...
        mongoLog = self.mongo.getbyDeviceTime(deviceTime, "electricity_meter")
    ...
    def test_07(self):
        ...
        data = electricity.JSONV2_Request("electricity-NotExist")
        deviceTime = data['deviceTime']
        data = json.dumps(data)
        res = OpenAPI(method=self.method, data=data)
        self.assertEqual(0, res.json()['code'])
    ...
    def test_08(self):
        """推送正式设备数据-设备未激活"""
        # mongo = pyMongodb.pymongo('172.16.0.137', 20200)
        data = electricity.JSONV2_Request("electricity-NotActive")
        deviceTime = data['deviceTime']
        data = json.dumps(data)
        res = OpenAPI(method=self.method, data=data)
        self.assertEqual(0, res.json()['code'])
    ...
    def test_09(self):
        "推送正式设备数据-项目已删除设备"
        data = electricity.JSONV2_Request("electricity-DelDevice")
        deviceTime = data['deviceTime']
    ...
    def test_10(self):
        "推送正式设备数据-项目删除应用"
        data = electricity.JSONV2_Request("electricity-DelApp")
        deviceTime = data['deviceTime']
        deviceId = data['deviceId']
        data = json.dumps(data)
        res = OpenAPI(method=self.method, data=data)
    ...
    def test_11(self):
        ...
        elkLog = elkSearch.elkDevice(deviceTime, self.method)
        self.assertEqual("usingCount字段超长", elkLog['exceptionContent'])
        self.assertEqual(deviceId, elkLog['deviceCode'])
        self.assertEqual(2, elkLog['uploadStatus'])
    """
