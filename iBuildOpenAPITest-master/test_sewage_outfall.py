...
from apps import sewage_outfall as sewage_data
dir_path = os.path.dirname(os.path.realpath(__file__))


class sewage_outfall(unittest.TestCase):
    def setUp(self):
    ...
    def test_04(self):
        ...
        elkLog = elkSearch.elkDevice(deviceTime, self.method)
        self.assertEqual("phValue字段不能为空", elkLog['exceptionContent'])
    ...
    def test_07(self):
        ...
        elkLog = elkSearch.elkDevice(deviceTime, self.method)
        self.assertEqual("phValue字段类型不正确", elkLog['exceptionContent'])
        self.assertEqual(2, elkLog['uploadStatus'])
    ...
    def test_09(self):
        """推送正式设备数据-设备ID不存在"""
        # mongo = pyMongodb.pymongo('172.16.0.137', 20200)
        data = sewage_data.JSONV2_Request("sewage-NotExist")
        deviceTime = data['deviceTime']
        data = json.dumps(data)
        res = OpenAPI(method=self.method, data=data)
        self.assertEqual(0, res.json()['code'])
    ...
    def test_10(self):
        """推送正式设备数据-设备未激活"""
        # mongo = pyMongodb.pymongo('172.16.0.137', 20200)
        data = sewage_data.JSONV2_Request("sewage-NotActive")
        deviceTime = data['deviceTime']
        data = json.dumps(data)
        res = OpenAPI(method=self.method, data=data)
        self.assertEqual(0, res.json()['code'])
